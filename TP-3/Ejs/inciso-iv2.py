...
class StereoTriangulationNode(Node):
    ...
    def callback(self, msg_l, msg_r):
        ...
        mask = (disparity > disparity.min()) & np.isfinite(disparity)
        ...
        if len(points) > 0:
         ...
         #genero y publico nube densa (global)
            ...
            points_global = (self.current_pose @ points_hom.T).T[:, :3] #trasnformo a cord globales con pose de gt y tomo solo x, y, z de los puntos
            ...
            if len(self.global_cloud) > 0:
                ...
                self.get_logger().info(f"Nube densa global publicada con {len(points_global)} puntos.")
...
# publicar tf de word <ros2 run tf2_ros static_transform_publisher 0 0 0 0 0 0 world mi_frame> o cambiar header.frame_id = "world"
